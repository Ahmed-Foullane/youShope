<x-sidebare-admin>
    <div class="bg-gray-50 h-screen flex flex-col items-center justify-center">
        ...
        <div class="bg-white p-8 rounded-xl shadow-xl w-full max-w-md">
            <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Add New Category</h2>
            <form action="{{ url('admin/store-category') }}" method="POST">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" class="w-full mt-1 p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500" value="{{ old('name') }}">
                    @error("name")
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="mt-4">
                    <button type="submit" class="w-full bg-purple-600 text-white p-3 rounded-md hover:bg-purple-700 focus:ring-2 focus:ring-purple-500">Add Category</button>
                </div>
            </form>
        </div>
    </div>
